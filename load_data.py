...
def make_csv_dataset(data_path, dataset_name):
    dataset_path = join(data_path, f"{dataset_name}.csv")
...
def load_data(data_path, dataset):
    # Handle CSV datasets
    csv_path = join(data_path, f"{dataset}.csv")
    if os.path.exists(csv_path):
        return make_csv_dataset(data_path, dataset)

    # Handle .npy pre-saved datasets
    try:
        ...
        raise FileNotFoundError(f"Dataset not found. Ensure the dataset '{dataset}' exists as a CSV file or in .npy format. Error: {e}")
